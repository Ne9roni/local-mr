...
local-mr is a local-first Git review tool. It combines the target branch, current `HEAD`, and worktree into a selectable comparison model, serves one review session on the loopback interface, and renders file diffs on demand in the browser.

## Core constraints

1. Never modify the real Git index of the repository being reviewed.
2. Include committed, staged, unstaged, and untracked files in the default comparison.
...
src/review-ui.html
  ...
  ├─ Switch between push versions and commit ranges
...
- `src/version-model.mjs` reads Git history, builds push, commit, and worktree versions, and snapshots the worktree through a temporary index.
...
- `src/review-ui.html` contains browser styles and interactions without requiring a build step.
...
Worktree comparisons use a temporary `GIT_INDEX_FILE`. The operation initializes it from `HEAD`, writes the worktree into that temporary index, and reads only a cached diff. The temporary directory is removed afterward, and the repository's real index is never refreshed.

The server maintains four bounded caches: version models, patches, full pages, and file fragments. Cache keys include `HEAD`, the target ref, upstream state, the worktree fingerprint, layout, or patch content. File metadata changes, a new `HEAD`, or a changed version selection invalidate the corresponding layer. Patch, page, and fragment caches are each capped at 64 MiB and also have independent entry-count limits.
...
- Browser tests use headless Chrome to cover Markdown sanitization, Mermaid, side-by-side and line-by-line diffs, and context expansion.
