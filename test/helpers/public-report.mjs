...
export const publicTestReport = (value) => String(JSON.stringify(value, null, 2) ?? "")
    .replace(loopbackUrlPattern, "[local review URL redacted]")
    .replace(tokenizedReviewPathPattern, "/[review token redacted]/$1")
    .replace(workstationPathPattern, "[workstation path redacted]")
    .replace(emailPattern, "[email redacted]");

export const printPublicTestReport = (value) => console.log(publicTestReport(value));
