...
**Your merge request, before it exists.** Review your entire Git branch—including uncommitted files—in a local MR-style browser UI before you push.

local-mr brings committed, staged, unstaged, and untracked changes together on one review page. Move between push snapshots, commits, ranges, and a virtual worktree version without modifying the real Git index.

**No commit required · Loopback only · Real Git index untouched**

## Why local-mr?

- **Review the whole branch.** See committed and uncommitted work together instead of stitching several `git diff` commands together.
- **Review how the branch evolved.** Compare push snapshots, individual commits, contiguous ranges, or the current worktree.
...
Requires Linux or WSL, Git, and Node.js 22 or later.

Try it in any Git repository without installing globally:
...
For development, clone the repository and run `npm run install:link` instead. Set `LOCAL_MR_PREFIX` to change the source install prefix.

## Features

- Committed, staged, unstaged, and untracked changes in one review
- Directory tree with per-file read and unread state
- Push versions, individual commits, contiguous commit ranges, and worktree comparisons
...
Uninstallation preserves read state under `~/.local/state/local-mr`.

## Acknowledgements and license

Diff rendering is powered by [diff2html](https://github.com/rtfpessoa/diff2html). Markdown preview uses [marked](https://github.com/markedjs/marked), [DOMPurify](https://github.com/cure53/DOMPurify), and [Mermaid](https://github.com/mermaid-js/mermaid). See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for attribution.
